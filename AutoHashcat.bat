...
set POTFILE="AutoHashcat_POTFILE_"%JOBNAME%
...
(for %%a in (%RULES_LIST%) do (
    START "" /W %HASHCAT_BIN% -a 0 -m %HASH_TYPE% -w4 -r %RULES_DIRECTORY%%%a %HASHES_FILE% %WORDLIST% -O --potfile-path %POTFILE%.pot
)) 

(for %%a in (%MASK_LIST%) do (
    START "" /W %HASHCAT_BIN% -a 3 -m %HASH_TYPE% -w4 %HASHES_FILE% %MASKS_DIRECTORY%%%a -O --potfile-path %POTFILE%.pot
    START "" /W %HASHCAT_BIN% -a 6 -m %HASH_TYPE% -w4 %HASHES_FILE% %WORDLIST% %MASKS_DIRECTORY%%%a -O --potfile-path %POTFILE%.pot
    START "" /W %HASHCAT_BIN% -a 7 -m %HASH_TYPE% -w4 %HASHES_FILE% %MASKS_DIRECTORY%%%a %WORDLIST% -O --potfile-path %POTFILE%.pot
)) 

START "" /W %HASHCAT_BIN% -i -a 3 -m %HASH_TYPE% -w4 %HASHES_FILE% ?a?a?a?a?a?a?a -O --potfile-path %POTFILE%.pot
